...
def train(gpu, args):
    ...
    if is_distributed:
        ...
        job_per_host = args.num_gpus
        if job_per_host == 0:
            job_per_host = 1
        world_size = len(args.hosts) * job_per_host
        os.environ["WORLD_SIZE"] = str(world_size)
        rank = args.hosts.index(args.current_host) * job_per_host + gpu
...
if __name__ == "__main__":
    ...
    if len(args.hosts) > 1 and args.num_gpus > 0:
