import os
cmd = "pip install -r requirements2.txt -i https://opentuna.cn/pypi/web/simple/"
os.system(cmd)
...
def parse_args():
    ...
    parser.add_argument('--data_path', type=str, default='../dataset/mvtec_anomaly_detection/bottle')
...
def main():
    ...
    train_feature_filepath = os.path.join(args.save_path, 'temp_%s' % args.arch, 'train.pkl')
...
def plot_fig(test_img, scores, gts, threshold, save_dir):
    ...
    for i in range(num):
        ...
        ax_img[4].imshow(gt, cmap='gray')
        ax_img[4].title.set_text('GroundTruth')
        left = 0.92
        bottom = 0.15
        width = 0.015
        height = 1 - 2 * bottom
